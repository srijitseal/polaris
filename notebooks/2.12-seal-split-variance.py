...
def _generate_combined_figures(output_dir: Path, dpi: int) -> None:
    ...
    # ── Combined panel: 2 subplots (random, cluster), XGBoost vs CheMeleon with error bars
    ...
    for ax, strategy in zip(axes, STRATEGY_ORDER):
        for i, (model_name, agg_df) in enumerate([("xgboost", xgb_df), ("chemeleon", chemeleon_df)]):
            strat_df = agg_df[agg_df["strategy"] == strategy]
            means, stds = [], []
            for ep in active_endpoints:
                row = strat_df[strat_df["endpoint"] == ep]
                if row.empty:
                    means.append(np.nan)
                    stds.append(0)
                else:
        ...
        ax.set_title(f"{strategy.capitalize()} split (5 repeats)", fontsize=14, fontweight="bold")
        ax.axhline(y=1, color="gray", linestyle="--", alpha=0.3)
        ax.legend(fontsize=9)

    axes[0].set_ylabel("RAE")
    fig.tight_layout()
    fig.savefig(combined_dir / "split_variance_rae_combined.png", dpi=dpi, bbox_inches="tight")
    plt.close("all")
    logger.info(f"Saved {combined_dir / 'split_variance_rae_combined.png'}")
