...
| DONE (XGB + CheMeleon) | `2.09-seal-iid-vs-ood-series.py` | IID vs OOD on chemical series (paper Fig 5, Table 3): Train on largest Butina cluster (temporal 80/20), OOD test on second cluster. R² negative on 5/8 endpoints (XGB) and 6/8 (CheMeleon) |
