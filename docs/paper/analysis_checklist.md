...
| DONE | `2.08-seal-baseline-performance.py` | Baseline performance on original competition split (paper Fig 6b): XGBoost (per-endpoint tuned via HalvingRandomSearchCV) on ECFP4 + full RDKit 2D descriptors + dimorphite_dl protonation, log-transform, competition metrics; MA-RAE 0.821 |
...
| DONE | `2.11-seal-scaffold-vs-random.py` | Scaffold vs random split comparison (paper Fig S1, ref: Greg Landrum's blog): implement naive Bemis-Murcko scaffold split, compare performance to random split — show they are similar without careful considerations, compare to cluster-based split to demonstrate when scaffold splits fail, quantify similarity, KS tests on 1-NN distance distributions |
| DONE | `2.12-seal-split-variance.py` | Split variance study (paper Fig S2, ref: Pat's blog): 20 random + 5 cluster repeats show random splits give false confidence (tight RAE std 0.002–0.012 at optimistic ~0.48) while cluster splits reveal true uncertainty (RAE std 0.020–0.060 at honest ~0.71); splitting strategy matters more than number of repeats; Mann-Whitney U tests on RAE distributions |
| DONE | `2.13-seal-molecular-variants.py` | Molecular variant consistency (paper Fig S4, ref: Srijit's LinkedIn): identify groups of related molecules (stereoisomers, scaffold decorations), compute intra-group fingerprint distances and prediction variance, test whether models learn chemistry vs memorize fingerprint artifacts |
